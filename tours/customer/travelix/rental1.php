...
<div class="super_container">
	...
	<!--	<div class="container">
		...
		$td=$_POST["tod"];
		$dis=$_POST["dis"];
			$sql="SELECT * FROM `tbl_addtraveler` where district='$dis' and tid not in (select tid from `tbl_travelerbook` where status=0 and  ('$fd' between tbl_travelerbook.fromone and tbl_travelerbook.toone or '$td' between tbl_travelerbook.fromone and tbl_travelerbook.toone  or ('$fd' <= tbl_travelerbook.fromone  and '$td' >= tbl_travelerbook.toone ))  )";
  ...
  if($rowcount !=0 )
	...
	{	
		...
		while($row=mysqli_fetch_array($result))
		{
			...
			$bid=$row['tid'];
			$bname=$row['tname'];
			$bdestination=$row['tnumber'];
			$bdetails=$row['tfacilities'];
			$bseat=$row['tseat'];
			$bcoast=$row['tcoast'];
		
		
			$bsleeper=$row['tdetails'];	
            $bimage=$row['timage'];	
        ...
        ?>
								...
								<div class="row">
									<div class="col-lg-4">
										<div class="room_image"><img src="../../admins/<?php echo $bimage; ?>" style="width:300px"></div>
									</div>
									<div class="col-lg-7">
										<div class="room_content">
										

											<div class="room_title"><b><?php echo "name :". $bname; ?> </button></div>
											<div class="room_title"><?php echo "Vehicle number:". $bdestination ?></b></div>
										
											<div class="room_title"><?php echo $bcoast."  :per one kilometer"; ?></div>
										
											<div class="room_title"><?php echo $bseat ."seats "; ?></div>
											<div class="room_title"><?php echo $bsleeper; ?></div>
											<div class="room_title"><?php echo $bdetails; ?></div>
...
<div class="col-lg-8">	
										<div class="room_button">
											<div class="button book_button trans_200"><a href="details1.php?id=<?php echo $bid; ?>  && fd=<?php echo $fd; ?> && td=<?php echo $td; ?> ">BOOK NOW<span></span><span></span><span></span></a></div>
										</div>
									</div>
								</div></div>
									</div>	
