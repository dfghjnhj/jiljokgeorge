...
<body>
	...
	if (isset($_POST['pay'])) {
		...
		// $balancerooms = $rooms - $wantedrooms;
		// echo $hn;
		// data: {
		// 	delux:'delux',
		// 	datefirst:'',
		...
		// }
		//ajax start -------
		//  $sql = "select SUM(wantedrooms) from `tbl_roombook` where '$datefirst' between tbl_roombook.checkin and  tbl_roombook.checkout or '$datesecond' between tbl_roombook.checkin  and tbl_roombook.checkout or '$datefirst' <= tbl_roombook.checkin and '$datesecond' >= tbl_roombook.checkout";
		$sql = "select SUM(wantedrooms) from `tbl_roombook` where '$datefirst' between tbl_roombook.checkin and  tbl_roombook.checkout AND type = '$delux' or '$datesecond' between tbl_roombook.checkin  and tbl_roombook.checkout AND type = '$delux' or '$datefirst' <= tbl_roombook.checkin and '$datesecond' >= tbl_roombook.checkout AND type = '$delux'";
		...
		if ($bookedRooms > 0) {
			// totla room count
			$sqlRoomCount = 'SELECT ' . $delux . ' as count, ' . $delux . 'price as price FROM `hoteldetails` WHERE `hid` = "' . $hid . '"';
		...
		//	$price = $data['price'];
			...
			if (($totalRooms - $bookedRooms) <= 0) {
				//no rooms available
				echo '<script>window.alert("no rooms available");</script>';
				//	return;
				return false;
			} 
		...
		// calculate amount
				...
				//$amount = $price * $wantedrooms * $dateDiff;
				$insertQuery = mysqli_query($con, "INSERT INTO `tbl_roombook`(`hid`,`hname`,`checkin`,`checkout`,`name`,`wantedrooms`,`type`,`payment`) VALUES('$hid','$hn','$datefirst','$datesecond','$bpname','$wantedrooms','$delux','not succes')");
		...
		if (isset($_POST['submit'])) {
			...
			if ($rowcount != 0) {
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					echo $amount;
					if ($amount1 < $amount) {
						echo "<script>alert('not suffient balance!');window.location.href='roomfill.php';</script>";
					} else {
						$amt = $amount1 - $amount;
						...
						echo "<script>alert('succeesfuly Booked!');window.location.href='search.php';</script>";
					}
				}
			} else {
				echo "<script>alert(' Paiment not succeesfuly!');window.location.href='roomfill.php';</script>";
			}
		}
	}
		?>
														...
														<div class="menu trans_500">
															<div class="menu_content d-flex flex-column align-items-center justify-content-center text-center">
																...
																<ul>
																	...
																	<li class="menu_item"><a href="about.html">about us</a>
																	</li>
																	<li class="menu_item"><a href="offers.html">offers</a>
																	</li>
																	<li class="menu_item"><a href="blog.html">news</a></li>
																	<li class="menu_item"><a href="contact.html">contact</a>
																	</li>
																</ul>
															</div>
														</div>
