...
//    echo  $next_date_object = date_modify($today,'-1 day');
    ...
    {
        echo "<script>alert('canceled!');window.location.href='familytourbooking.php';</script>";
        $sql1="update tbl_pb set status='1' where pbid='$id'";
     $resul1=mysqli_query($con,$sql1);
     $sqlone="select pid from `tbl_pb` where pbid='$id'";
    ...
    {
        echo "<script>alert('cancelation not possible! CANCELATION ONLY ALLOWED before TWO  DAY');window.location.href='familytourbooking.php';</script>";  
    }
    ?>