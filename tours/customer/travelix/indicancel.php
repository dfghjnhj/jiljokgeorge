...
//    echo  $next_date_object = date_modify($today,'-1 day');
    ...
    {
        echo "<script>alert('BOOKING CANCELD!');window.location.href='inditourbook.php';</script>";
        $sql1="update tbl_pbin set status='1' where pbiid='$id'";
     $resul1=mysqli_query($con,$sql1);
     $sqlone="select pid from `tbl_pbin` where pbiid='$id'";
    ...
    {
        echo "<script>alert('cancelation not possible! CANCELATION ONLY ALLOWED before TWO DAY');window.location.href='inditourbook.php';</script>";  
    }
    ?>